...
export default function AdminExamsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const [exams, countryRows] = await Promise.all([fetchAdminExams(), fetchCountries()]);
        if (!mounted) return;
        setRows(exams);
  ...
  function openEdit(item: AdminExam) {
    setEditing(item);
    setForm({
      name: item.name || '',
      links: item.links || '',
      duration: item.duration || '',
      examDate: toInputDate(item.examDate),
  ...
  async function submit() {
    ...
    if (!name || !countryId || Number.isNaN(countryId)) {
      setError('Exam name and country are required');
      return;
    }
    ...
    try {
      if (editing) {
        const updated = await updateAdminExam(editing.id, {
          name,
  ...
  async function remove(item: AdminExam) {
    const ok = window.confirm(`Delete exam "${item.name}"?`);
    if (!ok) return;

    try {
      await deleteAdminExam(item.id);
      ...
      <Box className="admin-panel">
        <Box className="admin-panel__head">
          <Typography variant="h6" fontWeight={800} sx={{ display: 'flex', alignItems: 'center', gap: 0.7 }}>
            <QuizRounded sx={{ fontSize: 18, color: '#0369a1' }} />
            Exams
          </Typography>
          <Stack direction="row" spacing={1}>
            <Chip label={loading ? 'Loading...' : `${rows.length} exams`} color="info" variant="outlined" />
            ...
            bodyRows={
              loading
                ? Array.from({ length: 8 }).map((_, idx) => (
                    <TableRow key={`exam-skeleton-${idx}`}>
                      <TableCell colSpan={7}>
                        <Skeleton height={30} />
                      </TableCell>
                    </TableRow>
                ...
                : pagedRows.map((item) => (
                    <TableRow hover key={item.id}>
                      ...
                      <TableCell>{item.examDate ? new Date(item.examDate).toLocaleDateString() : '-'}</TableCell>
                      <TableCell>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <FormControl fullWidth>
              <InputLabel id="exam-country-label">Country</InputLabel>
              <Select
                labelId="exam-country-label"
                label="Country"
