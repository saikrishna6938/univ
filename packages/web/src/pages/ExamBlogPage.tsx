...
import { fetchExamById, type AdminExam } from '../lib/api';
...
export default function ExamBlogPage() {
  const params = useParams();
  const navigate = useNavigate();
  const [item, setItem] = useState<AdminExam | null>(null);
  ...
  const summaryPoints = useMemo(
    () => [
      ...
      { label: 'Exam Date', value: formatDate(item?.examDate), icon: <CalendarMonthRounded sx={{ fontSize: 18 }} /> }
    ],
    [item]
  );
  ...
  return (
    <Box sx={{ display: 'grid', gap: 2.25 }}>
      ...
      <Typography variant="body2" sx={{ color: '#64748b' }}>
        {`Home / Exams / ${loading ? '...' : item?.countryName || '-'} / ${loading ? '...' : item?.name || '-'}`}
      </Typography>
      ...
      <Card sx={{ borderRadius: 1, border: '1px solid #d9e2ec', overflow: 'hidden' }}>
        ...
        >
          {loading ? (
            ...
            >
              {item?.name || 'Exam Details'}
            </Typography>
          )}

          <Typography variant="body1" sx={{ mt: 1.2, color: '#cbd5e1', maxWidth: 900 }}>
            Country specific exam guidance, timeline and official links in one place.
          </Typography>

          <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: 'wrap' }}>
            <Chip
              icon={<QuizRounded sx={{ fontSize: 18 }} />}
              label={loading ? 'Loading...' : 'Exam Overview'}
              variant="outlined"
        ...
        <Card sx={{ borderRadius: 1, border: '1px solid #d9e2ec' }}>
          <CardContent sx={{ p: { xs: 2, md: 3 } }}>
            <Stack direction="row" alignItems="center" spacing={1}>
              ...
              <Typography variant="h6" fontWeight={800}>Exam Overview</Typography>
            </Stack>
            <Divider sx={{ my: 1.4 }} />
            {loading ? (
              <Stack spacing={1}>
              ...
              <Typography variant="body1" sx={{ color: '#334155', lineHeight: 1.75, whiteSpace: 'pre-line' }}>
                {item?.description || 'No description provided for this exam yet.'}
              </Typography>
            )}
          </CardContent>
        </Card>
